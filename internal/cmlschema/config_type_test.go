...
func TestConfigType_String(t *testing.T) {
	...
	if cfg != "cmlschema.ConfigType" {
		t.Errorf("ConfigType.String() unexpected result %s", cfg)
	}
}
...
func TestNormalizedTypeValueFromTerraform(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		"true": {
			...
			expectation: NewConfigValue(`{"hello":"world"}`),
		},
		"unknown": {
			in:          tftypes.NewValue(tftypes.String, tftypes.UnknownValue),
			expectation: NewConfigUnknown(),
		},
		"null": {
			in:          tftypes.NewValue(tftypes.String, nil),
			expectation: NewConfigNull(),
		},
		"wrongType": {
	...
	for name, testCase := range testCases {
		...
		t.Run(name, func(t *testing.T) {
			...
			got, err := ConfigType{}.ValueFromTerraform(ctx, testCase.in)
			if err != nil {
